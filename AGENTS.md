...
- src/core/backend/bindings.ts
  ...
  - Source of truth: Rust types in openrisk-core and commands in src-tauri/src/**.
  - Regenerate with: cargo test -p openrisk-tauri export_bindings -- --nocapture (from src-tauri).

- src-tauri/crates/openrisk-core/schemas/plugin-manifest.schema.rs
  - Generated from JSON schema.
  - Source of truth: src-tauri/crates/openrisk-core/schemas/plugin-manifest.schema.json.
  - Do not hand-edit .schema.rs.

## How To Implement Backend API Changes Safely

1. Update shared domain/use-case types in src-tauri/crates/openrisk-core or adapter commands in src-tauri/src/**.
...
3. Regenerate TS bindings from Rust (never patch bindings.ts by hand):
   - cd src-tauri && cargo test -p openrisk-tauri export_bindings -- --nocapture
4. Build frontend:
   - cd .. && npm run build
