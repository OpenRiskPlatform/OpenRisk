...
final class OpenRiskAppModel: ObservableObject {
    @Published private(set) var project: NativeProjectSummary?
    @Published private(set) var scans: [NativeScanSummary] = []
    @Published private(set) var plugins: [NativePlugin] = []
    @Published private(set) var selectedScanID: String?
    @Published private(set) var selectedScanDetail: NativeScanDetail?
    @Published var errorMessage: String?
    @Published private(set) var isLoadingProject = false
    @Published private(set) var isLoadingScan = false
    @Published private(set) var isCreatingScan = false
    @Published private(set) var runningScanIDs: Set<String> = []
    @Published var pendingLockedProjectPath: String?
    @Published var scanPendingRename: NativeScanSummary?

    private let client = NativeOpenRiskClient()
    ...
    func openProject(at path: String, password: String? = nil) async {
        ...
        do {
            ...
            let loadedPlugins = try await client.loadPlugins()
