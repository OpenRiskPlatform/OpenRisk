...
extension OpenRiskAppModel {
    ...
    func reloadSettings() async {
        guard let settings = await performSettingsAction({
    ...
    func saveProjectSettings(
        ...
        guard await performSettingsAction({
    ...
    func installRegistryPlugin(_ plugin: NativeRegistryPlugin, version: String) async {
        let manifestURL = "https://raw.githubusercontent.com/OpenRiskPlatform/plugins/main/\(plugin.id)/\(version)/plugin.json"
        guard await performSettingsAction({
